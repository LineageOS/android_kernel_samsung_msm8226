...
#if defined(CONFIG_QPNP_CHARGER)
	pr_info("%s: status(%d), swell_mode(%d:%d), cv(%dmV), temp(%d)\n",
...
static void sec_bat_swelling_fullcharged_check(struct sec_battery_info *battery)
{
	...
	case SEC_BATTERY_FULLCHARGED_FG_CURRENT:
		if(battery->pdata->swelling_full_check_current) {
			if ((battery->current_now > 0 && battery->current_now <
					battery->pdata->swelling_full_check_current) &&
					(battery->current_avg > 0 && battery->current_avg <
					battery->pdata->swelling_full_check_current)) {
					value.intval = POWER_SUPPLY_STATUS_FULL;
			}
			       ...
			       union power_supply_propval *val)
{
	...
	if (((battery->wc_status) && battery->pdata->use_wireless_to_pogo)
			|| battery->cable_type == POWER_SUPPLY_TYPE_POGODOCK)
		val->intval = 1;
	else
		val->intval = 0;
...
#if defined(CONFIG_BATTERY_SWELLING)
	ret = of_property_read_u32(np, "battery,swelling_full_check_current",
		&pdata->swelling_full_check_current);
	if (ret) {
		pr_info("%s: swelling_full_check_current is Empty\n", __func__);
		pdata->swelling_full_check_current = 0;
	}
	ret = of_property_read_u32(np, "battery,swelling_high_rechg_voltage",
		&pdata->swelling_high_rechg_voltage);
	if (ret) {
